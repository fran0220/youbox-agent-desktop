...
export function registerGameStudioHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.gamestudio.CHECKPOINT, async (_ctx, workspaceId: string, projectId: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)

    const project = await checkpointGameProject(workspace.rootPath, projectId)
    log.info(`GAMESTUDIO_CHECKPOINT: Checkpointed game project ${projectId} in workspace ${workspaceId}`)
    broadcastChanged(workspaceId, projectId, 'updated')
    return project
  })

  server.handle(RPC_CHANNELS.gamestudio.RESTORE, async (_ctx, workspaceId: string, projectId: string, commit?: string | null) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)

    const project = await restoreGameProject(workspace.rootPath, projectId, commit)
    log.info(`GAMESTUDIO_RESTORE: Restored game project ${projectId} in workspace ${workspaceId}`)
    broadcastChanged(workspaceId, projectId, 'files')
    broadcastChanged(workspaceId, projectId, 'updated')
    return project
  })
}
