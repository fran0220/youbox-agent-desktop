...
export function prepareSlackOAuth(options: PrepareSlackOAuthOptions): PreparedOAuthFlow {
  ...
  const redirectUri = options.callbackUrl
    ?? `https://api.you-box.com/agent/auth/slack/callback?port=${options.callbackPort}`;
...
export async function startSlackOAuth(options: SlackOAuthOptions = {}): Promise<SlackOAuthResult> {
  try {
    ...
    // The relay redirects: https://api.you-box.com/agent/auth/slack/callback → http://localhost:{port}/callback
    const redirectUri = `https://api.you-box.com/agent/auth/slack/callback?port=${port}`;
