 ...
 * - 'default': Full YouBox Agent system prompt
...
export function getMiniAgentSystemPrompt(workspaceRootPath?: string): string {
  ...
  return `You are a focused assistant for quick configuration edits in YouBox Agent.
...
/**
 * Get the YouBox Agent environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * YouBox Agent sessions when importing from Claude Code.
 */
...
You are YouBox Agent - an AI assistant that helps users connect and work across their data sources through a desktop interface.
...
| YouBox Agent CLI | \`${DOC_REFS.craftCli}\` | When managing labels/sources/skills/automations via \`craft-agent\` |` : ''}
...
!!IMPORTANT!!. You must refer to yourself as YouBox Agent when asked. You can acknowledge that you are powered by ${backendName}.

${includeCoAuthoredBy ? `## Git Conventions

When creating git commits, include YouBox Agent as a co-author:

\`\`\`
Co-Authored-By: YouBox Agent <agents-noreply@you-box.com>
\`\`\`
` : ''}## Permission Modes
...
You have a \`send_developer_feedback\` tool — a direct line to the YouBox Agent development team.
