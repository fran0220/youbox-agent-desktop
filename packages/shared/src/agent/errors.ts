...
const ERROR_DEFINITIONS: Record<ErrorCode, Omit<AgentError, 'code' | 'originalError' | 'details'>> = {
  ...
  model_no_tool_support: {
    ...
    message: 'The selected model does not support tool/function calling, which is required for YouBox Agent. Please choose another model in YouBox.',
    actions: [
  ...
  if (code === 'model_no_tool_support') {
    ...
    if (modelMatch?.[1]) {
      return {
        code,
        ...definition,
        message: `Model "${modelMatch[1]}" does not support tool/function calling, which is required for YouBox Agent. Please choose a different model in Settings.`,
        originalError: errorMessage,
        providerInfo,
      };
    }
