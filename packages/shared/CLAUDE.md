...
- WebUI/headless source OAuth (Google/MCP/generic via `prepareOAuth` + `callbackUrl`) uses the deployment callback URL directly unless `CRAFT_OAUTH_RELAY_CALLBACK_URL` is set; then the provider sees that relay URI and the real target is carried in the relay `ca1.` state envelope (no Craft `agents.craft.do` default ships).
- Slack source OAuth requires HTTPS `redirect_uri`. WebUI passes `callbackUrl` directly; Electron uses `CRAFT_SLACK_OAUTH_RELAY_CALLBACK_URL` with `?port=` (no hardcoded Craft relay). When unset and only `callbackPort` is used, flows fail with a clear operator message.
