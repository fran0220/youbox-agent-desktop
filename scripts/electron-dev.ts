...
import { getInstanceConfigDir } from "../packages/shared/src/config/paths.ts";
...
function detectInstance(): void {
  ...
  if (match) {
    ...
    process.env.CRAFT_APP_NAME = `OriginAI [${instanceNum}]`;
    process.env.CRAFT_CONFIG_DIR = getInstanceConfigDir(instanceNum);
    process.env.CRAFT_DEEPLINK_SCHEME = `origincoworks${instanceNum}`;
...
function getElectronEnv(): Record<string, string> {
  ...
  return {
    ...
    CRAFT_APP_NAME: process.env.CRAFT_APP_NAME || "OriginAI",
    CRAFT_DEEPLINK_SCHEME: process.env.CRAFT_DEEPLINK_SCHEME || "origincoworks",
