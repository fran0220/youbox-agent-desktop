...
VERSIONS_URL="https://jaco.jingao.club/electron"
DOWNLOAD_DIR="$HOME/.originai/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="OriginAI-x64.AppImage"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.origincoworks.next"
    if pgrep -x "OriginAI" >/dev/null 2>&1; then
        info "Quitting OriginAI..."
        ...
        if pgrep -x "OriginAI" >/dev/null 2>&1; then
            ...
            pkill -9 -x "OriginAI" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  OriginAI has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'OriginAI'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.originai/app"
    WRAPPER_PATH="$INSTALL_DIR/originai"
    APPIMAGE_INSTALL_PATH="$APP_DIR/OriginAI-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "OriginAI.*AppImage" >/dev/null 2>&1; then
        info "Stopping OriginAI..."
        pkill -f "OriginAI.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# OriginAI launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.originai/app/OriginAI-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/OriginAI"
ELECTRON_CACHE_ALT="$HOME/.cache/OriginAI"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: OriginAI not found at $APPIMAGE_PATH"
    echo "Reinstall: curl -fsSL https://jaco.jingao.club/install-app.sh | bash"
    exit 1
fi
...
# AppImage creates a new /tmp/.mount_OriginAI-XXXX each launch, so any cached path is stale
for cache_dir in "$ELECTRON_CACHE" "$ELECTRON_CACHE_ALT"; do
    if [ -d "$cache_dir" ] && grep -rq '/tmp/\.mount_OriginAI' "$cache_dir" 2>/dev/null; then
        rm -rf "$cache_dir"
    fi
done
