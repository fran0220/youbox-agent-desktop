...
import { getConfigDir, getWorkspaces, getWorkspaceByNameOrId, loadStoredConfig, addWorkspace, saveConfig } from '@craft-agent/shared/config'
        ...
        createHandlerDeps: ({ sessionManager: sm, platform: p, oauthFlowStore: ofs }) => {
          ...
          messagingHandle = createMessagingBootstrap({
            ...
            getMessagingDir: (wsId: string) =>
              join(getConfigDir(), 'workspaces', wsId, 'messaging'),
...
app.on('window-all-closed', () => {
  ...
  void gameServerManager?.stopAll().catch(err => {
    mainLog.error('[game-server] stopAll failed during window-all-closed:', err)
  })
...
app.on('before-quit', async (event) => {
  ...
  if (sessionManager) {
    ...
    await gameServerManager?.stopAll()
