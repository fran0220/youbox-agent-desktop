 ...
 * Deep Link Handler
 *
 * Parses origincoworks:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   origincoworks://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   origincoworks://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   origincoworks://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   origincoworks://sources[/source/{sourceSlug}]          - Sources list
 *   origincoworks://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   origincoworks://action/{actionName}[/{id}][?params]
 *   origincoworks://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   origincoworks://allSessions/session/abc123                (specific session)
 *   origincoworks://settings/shortcuts                     (shortcuts page)
 *   origincoworks://sources/source/github                  (github source info)
 *   origincoworks://action/new-chat                        (uses active window)
 *   origincoworks://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   origincoworks://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
import { isProductDeepLinkProtocol } from '@craft-agent/shared/deeplink-scheme'
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (!isProductDeepLinkProtocol(parsed.protocol)) {
      return null
    }

    // For custom protocols, the hostname contains the first path segment
    // e.g., origincoworks://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., origincoworks://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // origincoworks://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    const COMPOUND_ROUTE_PREFIXES = [
      'allSessions', 'flagged', 'state', 'sources', 'settings', 'skills', 'studio', 'canvas', 'gamestudio', 'design'
    ]

    // origincoworks://allSessions/..., origincoworks://settings/..., etc. (compound routes)
    ...
    // origincoworks://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // origincoworks://action/... (no workspace - uses active window)
    if (host === 'action') {
