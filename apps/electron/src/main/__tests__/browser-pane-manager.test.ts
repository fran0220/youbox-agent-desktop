...
function createMockWebContents() {
  ...
  return {
    ...
    loadFile: mock(async (path: string, _opts?: unknown) => {
      const isToolbarPath = typeof path === 'string' && path.includes('browser-toolbar.html')
...
describe('BrowserPaneManager', () => {
  ...
  it('denies app deep-link popups and forwards to deep-link handler', async () => {
    ...
    await Bun.sleep(0)
    expect(mockShellOpenExternal).toHaveBeenCalledWith('origincoworks://settings')
  })
