...
2. Optionally, compare against the upstream project manually outside the `design://` runtime if the user explicitly wants a freshness check
...
**Upstream (live):** For users who want the very latest rules, the upstream project can be checked manually outside the bundled design runtime. The pinned snapshot is always the baseline.

## Usage

When a user provides a file or pattern argument:
1. Load [`references/guidelines.md`](references/guidelines.md) as the default source
2. Optionally compare against the upstream project outside the bundled design runtime if the user asks for a freshness check
